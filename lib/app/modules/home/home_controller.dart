...
class HomeController extends DefaultChangeNotifier {
  ...
  Future<void> loadTotalTAsks() async {
    ...
    todayTotalTasks = TotalTasksModel(
      ...
      totalTasksFinish: todayTasks.where((tasks) => tasks.finished).length,
    );

    tomorrowTotalTasks = TotalTasksModel(
      totalTasks: tomorrowTasks.length,
      totalTasksFinish: tomorrowTasks.where((tasks) => tasks.finished).length,
    );

    weekTotalTasks = TotalTasksModel(
      totalTasks: weekTasks.tasks.length,
      totalTasksFinish: weekTasks.tasks.where((tasks) => tasks.finished).length,
    );
  ...
  void filterByDay(DateTime date) {
    selectedDay = date;
    filteredTasks = allTasks.where((tasks) {
      return tasks.dateTime == date;
    }).toList();
    notifyListeners();
  }
  ...
  Future<void> checkOrUncheckTask(TaskModel task) async {
    ...
    await _tasksService.checkOrUnchekTask(taskUpdate);
